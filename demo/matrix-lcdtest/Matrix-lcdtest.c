...
static int set_color(int bpp, unsigned int color, int ystart, int yend) 
{
	...
	if (bpp == 16) {
		ushort *p = (ushort *) fbaddr+(ystart*xres);
...
int main(int argc, char *argv[]) 
{
	...
	if ((devname = getenv("LCDSET_FB_DEV")) == NULL) {
		devname = "/dev/fb0";
	}
	...
	} else if (argc == 2) {                    // draw color block
		color = strtoul(argv[1], 0, 0);
		set_color(bpp, color, 0, yres);
	} else {
	    switch(bpp) {
	    case 16:
            // only support fb-st7789s(BGR)
	        // red, bit[0-4]
